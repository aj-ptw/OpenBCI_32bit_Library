...
void OpenBCI_32bit_Library::activateAllChannelsToTestCondition(byte testInputCode, byte amplitudeCode, byte freqCode)
{
  ...
  int retVal = configureInternalTestSignal(amplitudeCode,freqCode);
  if (curErrorCodeMode == ERROR_CODE_MODE_ON && retVal < RESP_CONFIGURE_INTERNAL_TEST_SIGNAL_SUCCESS) {
    printAll(retVal); 
    sendEOT();
    return;
  }
  //change input type settings for all channels
  retVal = changeInputType(testInputCode);
  if (curErrorCodeMode == ERROR_CODE_MODE_ON && retVal < RESP_CHANGE_INPUT_TYPE_SUCCESS) {
    printAll(retVal); 
    sendEOT();
    return;
  }
...
int OpenBCI_32bit_Library::setChannelsToDefault(void){
...
int OpenBCI_32bit_Library::deactivateChannel(byte N) {
  byte confirmation, setting, startChan, endChan, targetSS;
  if(N < 9){
    targetSS = BOARD_ADS; startChan = 0; endChan = 8;
  }else{
...
int OpenBCI_32bit_Library::changeChannelLeadOffDetect()
{
  ...
  for(int b=0; b<2; b++){
    if(b == 0){ targetSS = BOARD_ADS; startChan = 0; endChan = 8; }
    if(b == 1){
      if(!daisyPresent){ return RESP_SUCCESS_CHANGE_CHANNEL_LEAD_OFF; }
      targetSS = DAISY_ADS; startChan = 8; endChan = 16;
    }
    ...
    if (curErrorCodeMode == ERROR_CODE_MODE_ON) {
      ...
      if (confirmation != P_setting) {
        if (targetSS == BOARD_ADS) {
          return RESP_FAILURE_CHANGE_CHANNEL_LEAD_OFF_SENSE_P_ON_BOARD;
        } else {
          return RESP_FAILURE_CHANGE_CHANNEL_LEAD_OFF_SENSE_P_DAISY;
        }
      }
    }
    ...
    if (curErrorCodeMode == ERROR_CODE_MODE_ON) {
      ...
      if (confirmation != N_setting) {
        if (targetSS == BOARD_ADS) {
          return RESP_FAILURE_CHANGE_CHANNEL_LEAD_OFF_SENSE_N_ON_BOARD;
        } else {
          return RESP_FAILURE_CHANGE_CHANNEL_LEAD_OFF_SENSE_N_DAISY;
        }
      }
    }
  }
...
int OpenBCI_32bit_Library::changeChannelLeadOffDetect(byte N)
{
...
int OpenBCI_32bit_Library::configureInternalTestSignal(byte amplitudeCode, byte freqCode)
{
  byte confirmation, setting, targetSS;
  for(int i=0; i<2; i++){
    if(i == 0){ targetSS = BOARD_ADS;}
    if(i == 1){
      if(daisyPresent == false){ return RESP_CONFIGURE_INTERNAL_TEST_SIGNAL_SUCCESS; }
      targetSS = DAISY_ADS;
    }
    ...
    if (curErrorCodeMode == ERROR_CODE_MODE_ON) {
      confirmation = RREG(CONFIG2,targetSS); delay(1);
      if (confirmation != setting) return RESP_CONFIGURE_INTERNAL_TEST_SIGNAL_FAILURE;
    }
    if (curBoardMode == BOARD_MODE_DEBUG || curDebugMode == DEBUG_MODE_ON) {
  ...
  return RESP_CONFIGURE_INTERNAL_TEST_SIGNAL_SUCCESS;
}

int OpenBCI_32bit_Library::changeInputType(byte inputCode){
  ...
  if (curErrorCodeMode == ERROR_CODE_MODE_ON) {
    for (int i = 1; i <= numChannels; i++) {
      int retVal = writeChannelSettings(i);
  ...
  return RESP_CHANGE_INPUT_TYPE_SUCCESS;
}
